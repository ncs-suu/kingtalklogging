...
class LoggingStarRating {
     ...
     * @param callback
     */
    static void showStarRating(Context context, final RatingCallback callback){
     ...
     * @param callback
     */
    private static void showStarRatingCustom(
        ...
        if(!(context instanceof Activity)) {
            if (Logging.sharedInstance().isLoggingEnabled()) {
                Log.e(Logging.TAG, "Can't show star rating dialog, the provided context is not based off a activity");
            }
            return;
        }

        LayoutInflater inflater = (LayoutInflater) context.getSystemService( Context.LAYOUT_INFLATER_SERVICE );
        View dialogLayout = inflater.inflate(R.layout.star_ratting_layout, null);
        ...
        ratingBar.setOnRatingBarChangeListener(new RatingBar.OnRatingBarChangeListener() {
            ...
            public void onRatingChanged(RatingBar ratingBar, float v, boolean b) {
                ...
                if(Logging.sharedInstance().getConsent(Logging.LoggingFeatureNames.starRating)) {
                    Map<String, String> segm = new HashMap<>();
                    segm.put("platform", "android");
                    segm.put("app_version", DeviceInfo.getAppVersion(context));
                    segm.put("rating", "" + rating);

                    Logging.sharedInstance().recordEvent(STAR_RATING_EVENT_KEY, segm, 1);
                }
    ...
    static class StarRatingPreferences {
        ...
        JSONObject toJSON() {
            ...
            try {
                ...
                if (Logging.sharedInstance().isLoggingEnabled()) {
                    Log.w(Logging.TAG, "Got exception converting an StarRatingPreferences to JSON", e);
                }
            }
        ...
        static StarRatingPreferences fromJSON(final JSONObject json) {
            ...
            if(json != null) {
                try {
                    ...
                    if (Logging.sharedInstance().isLoggingEnabled()) {
                        Log.w(Logging.TAG, "Got exception converting JSON to a StarRatingPreferences", e);
                    }
                }
            }
     ...
     */
    static void setStarRatingInitConfig(Context context, int limit, String starRatingTextTitle, String starRatingTextMessage, String starRatingTextDismiss) {
        StarRatingPreferences srp = loadStarRatingPreferences(context);
     ...
     * @param shouldShow
     */
    static void setShowDialogAutomatically(Context context, boolean shouldShow) {
        StarRatingPreferences srp = loadStarRatingPreferences(context);
     ...
     */
    static void setStarRatingDisableAskingForEachAppVersion(Context context, boolean disableAsking) {
        StarRatingPreferences srp = loadStarRatingPreferences(context);
     ...
     */
    static void registerAppSession(Context context, RatingCallback starRatingCallback) {
        StarRatingPreferences srp = loadStarRatingPreferences(context);
     ...
     */
    static int getAutomaticStarRatingSessionLimit(Context context){
        StarRatingPreferences srp = loadStarRatingPreferences(context);
     ...
     * @return
     */
    static int getCurrentVersionsSessionCount(Context context){
        StarRatingPreferences srp = loadStarRatingPreferences(context);
     ...
     * @param context
     */
    static void clearAutomaticStarRatingSessionCount(Context context){
        StarRatingPreferences srp = loadStarRatingPreferences(context);
        srp.sessionAmount = 0;
        saveStarRatingPreferences(context, srp);
    }
     ...
     * @param isCancellable
     */
    static void setIfRatingDialogIsCancellable(Context context, boolean isCancellable){
        StarRatingPreferences srp = loadStarRatingPreferences(context);
        srp.isDialogCancellable = isCancellable;
        saveStarRatingPreferences(context, srp);
    }

    /// Logging webDialog user rating
    ...
    static synchronized void showFeedbackPopup(final String widgetId, final String closeButtonText, final Activity activity, final Logging logging, final ConnectionQueue connectionQueue_, final FeedbackRatingCallback devCallback){
        if (Logging.sharedInstance().isLoggingEnabled()) {
            Log.d(Logging.TAG, "Showing Feedback popup for widget id: [" + widgetId + "]");
        }

        if(widgetId == null || widgetId.isEmpty()){
            if(devCallback != null){
                devCallback.callback("Logging widgetId cannot be null or empty");
            }
            throw new IllegalArgumentException("Logging widgetId cannot be null or empty");
        }

        if(logging.getConsent(Logging.LoggingFeatureNames.starRating)) {
            //check the device type
            final boolean deviceIsPhone;
            final boolean deviceIsTablet;
            final boolean deviceIsTv;

            deviceIsTv = LoggingStarRating.isDeviceTv(activity);

            if(!deviceIsTv) {
                deviceIsPhone = !LoggingStarRating.isDeviceTablet(activity);
                deviceIsTablet = LoggingStarRating.isDeviceTablet(activity);
            } else {
            ...
            try {
                ...
                if (Logging.sharedInstance().isLoggingEnabled()) {
                    Log.e(Logging.TAG, "IOException while checking for rating widget availability :[" + e.toString() + "]");
                }
            ...
            if (Logging.sharedInstance().isLoggingEnabled()) {
                Log.d(Logging.TAG, "rating widget url :[" + ratingWidgetUrl + "]");
            }
            ...
            (new ImmediateRequestMaker()).execute(urlConnection, false, new InternalFeedbackRatingCallback() {
                ...
                public void callback(JSONObject checkResponse) {
                    if(checkResponse == null){
                        if (Logging.sharedInstance().isLoggingEnabled()) {
                            Log.d(Logging.TAG, "Not possible to show Feedback popup for widget id: [" + widgetId + "], probably a lack of connection to the server");
                        }
                        if(devCallback != null){
    ...
    protected static class ImmediateRequestMaker extends AsyncTask<Object, Void, JSONObject> {
        ...
        protected JSONObject doInBackground(Object... params) {
            if (Logging.sharedInstance().isLoggingEnabled()) {
                Log.v(Logging.TAG, "Starting ImmediateRequestMaker request");
            }
            ...
            try {
                if(requestShouldBeDelayed){
                    ...
                    if (Logging.sharedInstance().isLoggingEnabled()) {
                        Log.v(Logging.TAG, "ImmediateRequestMaker request should be delayed, waiting for 10.5 seconds");
                    }
                    ...
                    try {
                        ...
                        if (Logging.sharedInstance().isLoggingEnabled()) {
                            Log.w(Logging.TAG, "While waiting for 10 seconds in ImmediateRequestMaker, sleep was interrupted");
                        }
                    }
                }
                ...
                if(wasSuccess) {
                    ...
                    if (Logging.sharedInstance().isLoggingEnabled()) {
                        Log.e(Logging.TAG, "Encountered problem while making a immediate server request, :[" + buffer.toString() + "]");
                    }
                    return null;
                }
     ...
     */
    private static boolean isDeviceTablet(Activity activity) {
