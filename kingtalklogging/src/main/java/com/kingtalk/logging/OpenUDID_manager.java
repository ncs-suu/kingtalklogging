...
class OpenUDID_manager implements ServiceConnection{
	...
	@Override
  	public void onServiceConnected(ComponentName className, IBinder service) {
		...
		try {
			...
			android.os.Parcel data = obtain();
			data.writeInt(mRandom.nextInt());
			android.os.Parcel reply = obtain();
			service.transact(1, obtain(), reply, 0);
	...
	public static void sync(Context context) {
		...
		if (OpenUDID == null) //Not found
		{
			...
			manager.mMatchingIntents = context.getPackageManager().queryIntentServices(new Intent("com.kingtalk.logging.OpenUDID.GETUDID"), 0);
