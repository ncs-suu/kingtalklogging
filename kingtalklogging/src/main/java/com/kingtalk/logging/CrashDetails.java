...
class CrashDetails {
    ...
    private static final int startTime = Logging.currentTimestamp();
     ...
     */
    private static String isInBackground() {
    ...
    static void addLog(String record) {
        ...
        if(recordLength > maxBreadcrumbSize){
            if(Logging.sharedInstance().isLoggingEnabled()){
                Log.d(Logging.TAG, "Breadcrumb exceeds character limit: [" + recordLength + "], reducing it to: [" + maxBreadcrumbSize + "]");
            }
        ...
        if(logs.size() > maxBreadcrumbLimit){
            if(Logging.sharedInstance().isLoggingEnabled()){
                Log.d(Logging.TAG, "Breadcrumb amount limit exceeded, deleting the oldest one");
            }
            logs.removeFirst();
        }
    }
     ...
     */
    private static JSONObject getCustomSegments() {
     ...
     */
    private static String getOpenGL(Context context) {
     ...
     */
    private static String getRamCurrent(Context context) {
     ...
     */
    @TargetApi(18)
    private static String getDiskCurrent() {
    ...
    private static String getBatteryLevel(Context context) {
        try {
            ...
            if (Logging.sharedInstance().isLoggingEnabled()) {
                Log.i(Logging.TAG, "Can't get batter level");
            }
        }
    ...
    private static String getRunningTime() {
        return Integer.toString(Logging.currentTimestamp() - startTime);
    }
     ...
     */
    private static String isOnline(Context context) {
        try {
            ...
            if (Logging.sharedInstance().isLoggingEnabled()) {
                Log.w(Logging.TAG, "Got exception determining connectivity", e);
            }
        }
        return null;
    }
     ...
     */
    private static String isMuted(Context context) {
