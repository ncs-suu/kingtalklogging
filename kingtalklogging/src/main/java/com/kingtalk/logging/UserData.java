...
class UserData {
    ...
    public static void setData(Map<String, String> data){
        ...
        if(picturePath != null){
            ...
            if (!sourceFile.isFile()) {
                if (Logging.sharedInstance().isLoggingEnabled()) {
                    Log.w(Logging.TAG, "Provided file " + picturePath + " can not be opened");
                }
                picturePath = null;
            }
        }
        ...
        if(data.containsKey(BYEAR_KEY)){
            try {
                ...
                if (Logging.sharedInstance().isLoggingEnabled()) {
                    Log.w(Logging.TAG, "Incorrect byear number format");
                }
                byear = 0;
            }
        }
     ...
     * @param value String with value for the property
     */
    private static void setCustomProperty(String key, String value){
        if(custom == null)
            custom = new HashMap<>();
     ...
     */
    private static void modifyCustomData(String key, double value, String mod){
        try {
            if(customMods == null)
     ...
     * @param mod String with modification command
     */
    private static void modifyCustomData(String key, String value, String mod){
        try {
            if(customMods == null)
                customMods = new HashMap<>();
            JSONObject ob;
    ...
    private static JSONObject toJSON() {
        ...
        try {
            ...
            if (Logging.sharedInstance().isLoggingEnabled()) {
                Log.w(Logging.TAG, "Got exception converting an UserData to JSON", e);
            }
        }
        ...
        if(json != null){
            ...
            if (!json.isNull(CUSTOM_KEY)) {
                ...
                try {
                    ...
                    if (Logging.sharedInstance().isLoggingEnabled()) {
                        Log.w(Logging.TAG, "Got exception converting an Custom Json to Custom User data", e);
                    }
                }
            }
        }
